...
const FUNNEL_COLORS = ['#e5e5e5', '#ffffff', '#a1a1aa', '#ffffff'];
...
export default function Analytics() {
  ...
  const kpis = analytics ? [
    { label: 'Chiffre d\'Affaires', value: `€${analytics.kpis.revenue.toLocaleString('fr-FR', {maximumFractionDigits:0})}`, change: '+18.2%', up: analytics.kpis.revenue > 0, color: '#ffffff',
      spark: analytics.revenueData?.map(d => Number(d.rev)) || [] },
    { label: 'Bénéfice Net', value: `€${analytics.kpis.profit.toLocaleString('fr-FR', {maximumFractionDigits:0})}`, change: analytics.kpis.profit >= 0 ? '+' : '', up: analytics.kpis.profit >= 0, color: '#ffffff',
      spark: analytics.revenueData?.map(d => Math.max(Number(d.rev) - Number(d.ads), 0)) || [] },
    { label: 'Commandes', value: analytics.kpis.ordersCount.toLocaleString('fr-FR'), change: '+24.1%', up: true, color: '#ffffff',
      spark: analytics.revenueData?.map(d => Number(d.rev) * 0.3) || [] },
    { label: 'Taux Conversion', value: `${analytics.kpis.convRate.toFixed(1)}%`, change: analytics.kpis.convRate > 3 ? '+0.5%' : '-0.3%', up: analytics.kpis.convRate > 3, color: '#ffffff',
      spark: analytics.revenueData?.map(() => analytics.kpis.convRate + (Math.random() - 0.5) * 2) || [] },
    { label: 'Panier Moyen', value: `€${analytics.kpis.aov.toFixed(2)}`, change: '+5.4%', up: true, color: '#ffffff',
      spark: analytics.revenueData?.map(() => analytics.kpis.aov + (Math.random() - 0.5) * 10) || [] },
  ] : [];

  const revData = analytics?.revenueData || [];
  const maxRev = Math.max(...revData.map(d => Math.max(Number(d.rev), Number(d.ads))), 1);

  const trafficSources = (analytics?.trafficSources || []).map(s => ({
    ...s,
    color: SOURCE_COLORS[s.name] || '#71717a',
  }));

  const deviceData = (analytics?.deviceData || []).map(d => ({
    ...d,
    color: DEVICE_COLORS[d.name] || '#71717a',
  }));
  ...
  const geoColors = ['#ffffff', '#ffffff', '#a1a1aa', '#a1a1aa', '#ffffff', '#e5e5e5', '#71717a', '#e5e5e5'];
  ...
  return (
    <div className={`${styles.container} animate-fade-in`}>
      ...
      <div className={styles.grid2col}>
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><BarChart3 size={18} color="#ffffff" /> Revenus vs Dépenses Pub</h2>
            <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <span style={{ width: 10, height: 10, borderRadius: 3, background: 'linear-gradient(to top, #ffffff, #ffffff)', display: 'inline-block' }}></span> Revenus
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <span style={{ width: 10, height: 10, borderRadius: 3, background: 'linear-gradient(to top, #71717a, #a1a1aa)', display: 'inline-block' }}></span> Dépenses
              </span>
            </div>
          </div>
                ...
                <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', position: 'relative', width: '100%', justifyContent: 'center' }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      background: 'linear-gradient(to top, rgba(255,255,255,0.05), #a1a1aa)',
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Activity size={18} color="#ffffff" /> Funnel de Conversion</h2>
          </div>
      ...
      <div className={styles.grid3col}>
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Globe size={18} color="#e5e5e5" /> Sources de Trafic</h2>
          </div>
          <DonutChart
            data={trafficSources}
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Smartphone size={18} color="#e5e5e5" /> Appareils</h2>
          </div>
          <DonutChart
            data={deviceData}
            centerValue={fmt(totalTraffic)}
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Globe size={18} color="#ffffff" /> Géographie</h2>
          </div>
          <DonutChart
            data={geoData}
            centerValue="5"
      ...
      <div className={styles.grid2col}>
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><BarChart2 size={18} color="#e5e5e5" /> Campagnes Publicitaires</h2>
          </div>
              ...
              {adCampaigns.map((ad, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td style={{ color: '#71717a' }}>€{ad.spend.toLocaleString('fr-FR')}</td>
                    <td>{ad.impressions.toLocaleString('fr-FR')}</td>
                    <td>{ad.clicks.toLocaleString('fr-FR')}</td>
                    <td style={{ color: '#ffffff', fontWeight: 600 }}>{ad.conversions}</td>
                    <td style={{ color: '#ffffff', fontWeight: 700 }}>{roas}x</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Clock size={18} color="#e5e5e5" /> Activité en Direct</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span className={styles.statusDot} style={{ background: '#ffffff' }} />
                <div>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#ffffff' }}>✓ Live</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span className={styles.statusDot} style={{ background: '#e5e5e5' }} />
                <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span className={styles.statusDot} style={{ background: adCampaigns.length > 0 ? '#ffffff' : '#a1a1aa' }} />
                <div>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#ffffff' }}>Actif</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Target size={18} color="#a1a1aa" /> Objectifs Mensuels</h2>
        </div>
        <div className={styles.progressList}>
          {[
            { name: `Objectif CA (€10,000)`, pct: Math.min(Math.round((analytics?.kpis.revenue || 0) / 10000 * 100), 100), color: '#ffffff' },
            { name: `Objectif Commandes (300)`, pct: Math.min(Math.round((analytics?.kpis.ordersCount || 0) / 300 * 100), 100), color: '#e5e5e5' },
            { name: `Objectif Visiteurs (5000)`, pct: Math.min(Math.round(totalTraffic / 5000 * 100), 100), color: '#a1a1aa' },
            { name: `Objectif Taux Conversion (5%)`, pct: Math.min(Math.round((analytics?.kpis.convRate || 0) / 5 * 100), 100), color: '#a1a1aa' },
          ].map((obj, i) => (
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Flame size={18} color="#a1a1aa" /> Carte de Chaleur — Ventes par Heure</h2>
        </div>
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><BarChart2 size={18} color="#e5e5e5" /> Performances Publicitaires</h2>
        </div>
            ...
            return (
              <div key={i} className={styles.adCard}>
                <div className={styles.adCardHeader}>
                  ...
                  <span className={styles.adStatusBadge} style={{ background: ad.status === 'ACTIVE' ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)', color: ad.status === 'ACTIVE' ? '#ffffff' : '#a1a1aa' }}>
                ...
                <div className={styles.adMetricRow} style={{ borderBottom: 'none' }}><span>ROAS</span><span className={styles.adMetricValue} style={{ color: '#ffffff' }}>{roas}x</span></div>
              </div>
            );
          })}
        </div>
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Users size={18} color="#a1a1aa" /> Insights Clients</h2>
        </div>
        <div className={styles.customerGrid}>
          {[
            { val: fmt(totalTraffic), label: 'Visiteurs Uniques', color: '#ffffff' },
            { val: `€${analytics?.kpis.aov.toFixed(2) || '0'}`, label: 'Panier Moyen', color: '#ffffff' },
            { val: `${analytics?.kpis.convRate.toFixed(1) || '0'}%`, label: 'Taux de Conversion', color: '#e5e5e5' },
            { val: `${analytics?.kpis.ordersCount || 0}`, label: 'Commandes Totales', color: '#ffffff' },
          ].map((m, i) => (
      ...
      <div className={styles.grid2even}>
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Activity size={18} color="#ffffff" /> Scores de Santé</h2>
          </div>
          <div className={styles.gaugeContainer}>
            {[
              { label: 'Score Boutique', val: Math.min(Math.round(analytics?.kpis.convRate * 20 || 0), 100), color: '#ffffff' },
              { label: 'Santé Pub', val: adCampaigns.length > 0 ? 78 : 0, color: '#e5e5e5' },
              { label: 'Remplissage', val: Math.min(Math.round((analytics?.kpis.ordersCount / 300) * 100), 100), color: '#ffffff' },
              { label: 'Trafic', val: Math.min(Math.round((totalTraffic / 5000) * 100), 100), color: '#a1a1aa' },
            ].map((g, i) => (
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><AlertTriangle size={18} color="#71717a" /> Résumé Financier</h2>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              { name: 'Revenus Totaux', val: `€${analytics?.kpis.revenue.toFixed(0) || 0}`, color: '#ffffff' },
              { name: 'Dépenses Pub', val: `€${adCampaigns.reduce((s,a) => s + a.spend, 0).toFixed(0)}`, color: '#71717a' },
              { name: 'Bénéfice Net', val: `€${analytics?.kpis.profit.toFixed(0) || 0}`, color: analytics?.kpis.profit >= 0 ? '#ffffff' : '#71717a' },
              { name: 'Panier Moyen', val: `€${analytics?.kpis.aov.toFixed(2) || 0}`, color: '#ffffff' },
              { name: 'Taux de Conversion', val: `${analytics?.kpis.convRate.toFixed(1) || 0}%`, color: '#e5e5e5' },
            ].map((s, i) => (
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Mail size={18} color="#ffffff" /> Email Marketing</h2>
        </div>
        <div className={styles.emailGrid}>
          {[
            { val: fmt(analytics?.emailData?.subscribers || 0), label: 'Abonnés Liste', color: '#ffffff' },
            { val: `${analytics?.emailData?.openRate?.toFixed(1) || 0}%`, label: 'Taux d\'Ouverture', color: '#ffffff' },
            { val: `${analytics?.emailData?.clickRate?.toFixed(1) || 0}%`, label: 'Taux de Clic', color: '#ffffff' },
            { val: `€${Math.round(analytics?.emailData?.revenue || 0)}`, label: 'Revenus Email', color: '#a1a1aa' },
            { val: `${analytics?.emailData?.unsubRate?.toFixed(1) || 0}%`, label: 'Taux Désabonnement', color: '#71717a' },
            { val: `${analytics?.emailData?.campaignsSent || 0}`, label: 'Campagnes Envoyées', color: '#e5e5e5' },
          ].map((e, i) => (
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Heart size={18} color="#a1a1aa" /> Engagement Réseaux Sociaux</h2>
        </div>
        <div className={styles.grid2even}>
          <DonutChart
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', justifyContent: 'center' }}>
            {[
              { icon: Heart, label: 'Likes ce mois', val: fmt(analytics?.socialData?.likes || 0), color: '#a1a1aa' },
              { icon: MessageCircle, label: 'Commentaires', val: fmt(analytics?.socialData?.comments || 0), color: '#e5e5e5' },
              { icon: Share2, label: 'Partages', val: fmt(analytics?.socialData?.shares || 0), color: '#ffffff' },
              { icon: Eye, label: 'Portée organique', val: fmt(analytics?.socialData?.reach || 0), color: '#ffffff' },
              { icon: ThumbsUp, label: 'Taux Engagement', val: `${analytics?.socialData?.engagementRate?.toFixed(1) || 0}%`, color: '#a1a1aa' },
              { icon: Hash, label: 'Mentions', val: fmt(analytics?.socialData?.mentions || 0), color: '#ffffff' },
            ].map((s, i) => {
              const Icon = s.icon;
              return (
                <div key={i} style={{
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Truck size={18} color="#a1a1aa" /> Expéditions & Fulfillment</h2>
        </div>
        <div className={styles.customerGrid}>
          {[
            { val: `${analytics?.kpis.ordersCount || 0}`, label: 'Commandes ce mois', color: '#ffffff' },
            { val: `${Math.round((analytics?.kpis.ordersCount || 0) * 0.87)}`, label: 'Expédiées', color: '#ffffff' },
            { val: `${Math.round((analytics?.kpis.ordersCount || 0) * 0.09)}`, label: 'En cours', color: '#e5e5e5' },
            { val: `${Math.round((analytics?.kpis.ordersCount || 0) * 0.04)}`, label: 'En attente', color: '#a1a1aa' },
          ].map((m, i) => (
            <div key={i} className={styles.customerMetric}>
              <div className={styles.customerMetricValue} style={{ color: m.color }}>{m.val}</div>
              <div className={styles.customerMetricLabel}>{m.label}</div>
        ...
        <div className={styles.progressList} style={{ marginTop: '20px' }}>
          {[
            { name: 'Taux d\'expédition à temps', pct: 94, color: '#ffffff' },
            { name: 'Livraisons réussies', pct: 97, color: '#e5e5e5' },
            { name: 'Colis retournés', pct: 3, color: '#71717a' },
          ].map((obj, i) => (
            <div key={i} className={styles.progressItem}>
              <div className={styles.progressTop}>
                <span className={styles.progressName}>{obj.name}</span>
