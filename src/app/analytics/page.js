...
export default function Analytics() {
  ...
  const kpis = analytics ? [
    { label: 'Chiffre d\'Affaires', value: `Ôé¼${analytics.kpis.revenue.toLocaleString('fr-FR', {maximumFractionDigits:0})}`, change: '+18.2%', up: analytics.kpis.revenue > 0, color: '#a78bfa',
      spark: analytics.revenueData?.map(d => Number(d.rev)) || [] },
    { label: 'B├®n├®fice Net', value: `Ôé¼${analytics.kpis.profit.toLocaleString('fr-FR', {maximumFractionDigits:0})}`, change: analytics.kpis.profit >= 0 ? '+' : '', up: analytics.kpis.profit >= 0, color: '#34d399',
    ...
    { label: 'Panier Moyen', value: `Ôé¼${analytics.kpis.aov.toFixed(2)}`, change: '+5.4%', up: true, color: '#f472b6',
    ...
    <div className={`${styles.container} animate-fade-in`}>
      ...
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <p>Bienvenue {firstName} ÔÇö donn├®es en temps r├®el depuis votre base de donn├®es.</p>
        </div>
      ...
      {/* KPI ROW ÔÇö 5 cartes avec donn├®es r├®elles */}
      ...
      <div className={styles.grid2col}>
        {/* Bar Chart: Revenus vs D├®penses Pub (donn├®es r├®elles) */}
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><BarChart3 size={18} color="#a78bfa" /> Revenus vs D├®penses Pub</h2>
            <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <span style={{ width: 10, height: 10, borderRadius: 3, background: 'linear-gradient(to top, #ef4444, #f97316)', display: 'inline-block' }}></span> D├®penses
              </span>
            </div>
          </div>
              ...
              <div key={i} className={styles.barCol}>
                <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', position: 'relative', width: '100%', justifyContent: 'center' }}>
                  <div
                    ...
                    title={`Revenus: Ôé¼${d.realRev?.toFixed(0) || d.rev}`}
                  />
                  <div
                    className={styles.bar}
                    style={{
                    ...
                    title={`Pub: Ôé¼${d.realAds?.toFixed(0) || d.ads}`}
                  />
                </div>
        ...
        {/* Funnel de Conversion (donn├®es r├®elles) */}
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
      ...
      {/* ROW 3: 3 DONUT CHARTS (donn├®es r├®elles) */}
      <div className={styles.grid3col}>
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Globe size={18} color="#10b981" /> G├®ographie</h2>
          </div>
          <DonutChart
            data={geoData}
            centerValue="5"
      ...
      {/* ROW 4: AD CAMPAIGNS TABLE (donn├®es r├®elles) + ACTIVITY */}
      <div className={styles.grid2col}>
        {/* Top Ad Campaigns */}
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
              ...
              {adCampaigns.map((ad, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td style={{ color: '#f87171' }}>Ôé¼{ad.spend.toLocaleString('fr-FR')}</td>
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Clock size={18} color="#60a5fa" /> Activit├® en Direct</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: '#fff' }}>Base de donn├®es connect├®e</div>
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>PostgreSQL via Prisma</div>
                </div>
              </div>
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#34d399' }}>Ô£ô Live</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span className={styles.statusDot} style={{ background: '#3b82f6' }} />
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: '#fff' }}>{analytics?.kpis.ordersCount || 0} commandes trait├®es</div>
              ...
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#fff' }}>Ôé¼{analytics?.kpis.revenue.toFixed(0) || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>D├®pense totale: Ôé¼{adCampaigns.reduce((s,a) => s + a.spend, 0).toFixed(0)}</div>
                </div>
              </div>
      ...
      {/* ROW 5: PROGRESS BARS ÔÇö Objectifs Mensuels */}
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
        ...
        <div className={styles.progressList}>
          {[
            { name: `Objectif CA (Ôé¼10,000)`, pct: Math.min(Math.round((analytics?.kpis.revenue || 0) / 10000 * 100), 100), color: '#a855f7' },
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Flame size={18} color="#f97316" /> Carte de Chaleur ÔÇö Ventes par Heure</h2>
        </div>
              ...
              {Array.from({ length: 24 }, (_, hi) => {
                ...
                return (
                  <div
                    ...
                    title={`${day} ${hi}h ÔÇö ${Math.round(val * 12)} ventes`}
                  />
                );
              })}
            </>
      ...
      {/* ROW 7: AD PERFORMANCE CARDS (donn├®es r├®elles) */}
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
        ...
        <div className={styles.adGrid}>
          {adCampaigns.map((ad, i) => {
            ...
            const roas = ad.spend > 0 ? ((ad.conversions * (analytics?.kpis.aov || 50)) / ad.spend).toFixed(1) : '0';
            const icons = { Facebook: '­ƒôÿ', TikTok: '­ƒÄÁ', Google: '­ƒöì' };
            return (
              <div key={i} className={styles.adCard}>
                <div className={styles.adCardHeader}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div className={styles.adPlatformIcon} style={{ background: 'rgba(255,255,255,0.05)' }}>{icons[ad.platform] || '­ƒôè'}</div>
                ...
                <div className={styles.adMetricRow}><span>D├®pens├®</span><span className={styles.adMetricValue}>Ôé¼{ad.spend.toLocaleString('fr-FR')}</span></div>
                ...
                <div className={styles.adMetricRow}><span>CPC Moyen</span><span className={styles.adMetricValue}>Ôé¼{cpc}</span></div>
      ...
      {/* ROW 8: CUSTOMER INSIGHTS (calcul├®) */}
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
        ...
        <div className={styles.customerGrid}>
          {[
            ...
            { val: `Ôé¼${analytics?.kpis.aov.toFixed(2) || '0'}`, label: 'Panier Moyen', color: '#10b981' },
      ...
      <div className={styles.grid2even}>
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><Activity size={18} color="#10b981" /> Scores de Sant├®</h2>
          </div>
          <div className={styles.gaugeContainer}>
            {[
              { label: 'Score Boutique', val: Math.min(Math.round(analytics?.kpis.convRate * 20 || 0), 100), color: '#10b981' },
              { label: 'Sant├® Pub', val: adCampaigns.length > 0 ? 78 : 0, color: '#3b82f6' },
        ...
        <div className={styles.sectionBox}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}><AlertTriangle size={18} color="#ef4444" /> R├®sum├® Financier</h2>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              { name: 'Revenus Totaux', val: `Ôé¼${analytics?.kpis.revenue.toFixed(0) || 0}`, color: '#10b981' },
              { name: 'D├®penses Pub', val: `Ôé¼${adCampaigns.reduce((s,a) => s + a.spend, 0).toFixed(0)}`, color: '#ef4444' },
              { name: 'B├®n├®fice Net', val: `Ôé¼${analytics?.kpis.profit.toFixed(0) || 0}`, color: analytics?.kpis.profit >= 0 ? '#10b981' : '#ef4444' },
              { name: 'Panier Moyen', val: `Ôé¼${analytics?.kpis.aov.toFixed(2) || 0}`, color: '#a855f7' },
      ...
      <div className={styles.sectionBox}>
        ...
        <div className={styles.emailGrid}>
          {[
            { val: fmt(analytics?.emailData?.subscribers || 0), label: 'Abonn├®s Liste', color: '#06b6d4' },
            ...
            { val: `Ôé¼${Math.round(analytics?.emailData?.revenue || 0)}`, label: 'Revenus Email', color: '#f59e0b' },
            { val: `${analytics?.emailData?.unsubRate?.toFixed(1) || 0}%`, label: 'Taux D├®sabonnement', color: '#ef4444' },
            { val: `${analytics?.emailData?.campaignsSent || 0}`, label: 'Campagnes Envoy├®es', color: '#3b82f6' },
          ].map((e, i) => (
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Heart size={18} color="#ec4899" /> Engagement R├®seaux Sociaux</h2>
        </div>
        <div className={styles.grid2even}>
          <DonutChart
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', justifyContent: 'center' }}>
            {[
              ...
              { icon: Eye, label: 'Port├®e organique', val: fmt(analytics?.socialData?.reach || 0), color: '#a855f7' },
      ...
      <div className={styles.sectionBox}>
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle}><Truck size={18} color="#f59e0b" /> Exp├®ditions & Fulfillment</h2>
        </div>
        <div className={styles.customerGrid}>
          {[
            { val: `${analytics?.kpis.ordersCount || 0}`, label: 'Commandes ce mois', color: '#a855f7' },
            { val: `${Math.round((analytics?.kpis.ordersCount || 0) * 0.87)}`, label: 'Exp├®di├®es', color: '#10b981' },
        ...
        <div className={styles.progressList} style={{ marginTop: '20px' }}>
          {[
            { name: 'Taux d\'exp├®dition ├á temps', pct: 94, color: '#10b981' },
            { name: 'Livraisons r├®ussies', pct: 97, color: '#3b82f6' },
            { name: 'Colis retourn├®s', pct: 3, color: '#ef4444' },
          ].map((obj, i) => (
            <div key={i} className={styles.progressItem}>
              <div className={styles.progressTop}>
                <span className={styles.progressName}>{obj.name}</span>
